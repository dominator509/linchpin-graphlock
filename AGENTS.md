# AGENTS.md - LINCHPIN GraphLock Control Plane

## 1. Mission
Build LINCHPIN, a local-first Windows Patent Intelligence and Commercialization OS that helps an inventor discover evidence-backed opportunity whitespace, preserve human conception, research prior art, architect supported patent drafts, prepare filing packages, manage prosecution/deadlines, and commercialize intellectual property without faking legal certainty or exposing confidential invention data by default.
...
1. GraphLock laws in this file and `.agent/DONE_LAW.md`.
2. Product scope and contracts in `PROJECT_BRIEF.md`, `ARCHITECTURE.md`, `SECURITY.md`, and `.agent/specs/`.
3. Accepted ADRs in `DECISIONS.md`.
4. Active node ExecPlan in `.agent/execplans/`.
5. Repository implementation and lockfiles.
6. Hash-chained execution evidence in `.agent/state/LEDGER.jsonl`; `LEDGER.md` is only the readable view.
7. Agent prose, chat memory, and summaries are never authoritative evidence.

When sources conflict, higher level wins. Do not silently reconcile a conflict. Record it in `DECISIONS.md`, update the lower source under change control, invalidate affected evidence, and rerun descendants.

## 4. Graph protocol
- Work one graph node at a time unless `.agent/GRAPH.md` explicitly authorizes parallel read-only verification.
- A node closes only when every milestone is committed, its requirement/test/evidence rows are current, anti-gaming review passes, and all owned DoD clauses have an evidence-backed disposition.
- `CLOSED_BLOCKED` is a scheduling/accounting state, never success. Continue independent nodes.
- Production-code changes after a candidate is frozen require REMEDIATOR mode, a new candidate epoch, and reruns of affected descendants.
- Commit after each milestone with requirement IDs in the message.

## 5. STOP conditions
Stop only when: (a) preflight fails before a run; (b) the next action would destroy user/production data or create an irreversible external side effect not explicitly authorized; (c) a legal, financial, or security judgment is required and the specs do not answer it; (d) the active node reaches `CLOSED_BLOCKED` after the full bounded recovery ladder; or (e) a production deploy is reached while auto-deploy authorization is `no`, in which case the artifact may be ship-ready but deployment is MANUAL. Do not ask the user for next steps, preferences, or confirmation. Proceed with the smallest reversible option when none of these STOP conditions applies.

## 6. Anti-drift rules
- Every production path maps to a stable requirement ID or accepted ADR.
- No broad refactor, unrelated cleanup, provider addition, datastore addition, new cloud service, or change in legal workflow without change control.
- Run architecture drift accounting before node closure and release.
- Product claims in UI/docs must not exceed proven behavior.

## 7. Anti-hallucination rules
Never invent package APIs, CLI flags, provider capabilities, OAuth scopes, USPTO forms, endpoints, routes, tables, config keys, legal deadlines, fees, or model names. Verify repository facts locally and time-sensitive external facts from first-party documentation. Commands come only from `COMMANDS.md`. Unknowns are assumptions with a verification command and affected-node block.

## 8. Anti-fixation rules
For a failed command: 1) preserve exact failure evidence; 2) classify code, test, environment, dependency, credential, external, or authorization cause; 3) test a different falsifiable hypothesis; 4) make one bounded change; 5) rerun the smallest proving command; 6) after three distinct failed hypotheses, close the affected node BLOCKED with reproduction and unblock condition. Never repeat an unchanged failing action.

## 9. Reality law
Software that appears to work is a failure state. Only software proven by live-fire counts. Final proof crosses the actual desktop/UI or declared service boundary, reaches real persistence or a valid sandbox/external dependency, independently reads the effect back, exercises a negative case, and binds the result to the exact artifact digest.

## 10. Dependency rules
Prefer already-approved dependencies. Add a dependency only when the capability cannot be built safely with existing code or the standard library. Pin exact versions, update lockfiles, run license/security checks, record the rationale, and prove the dependency is actually exercised. Permissive-license-first policy applies to redistributed core.

## 11. File creation and commit rules
Create the failing acceptance oracle before non-trivial production implementation. Keep mutable production paths under one writer lease. Parallel agents use isolated worktrees and a merge queue. Never use force push or history rewriting in the GraphLock run.

## 12. Testing rules
Follow `TESTING.md` and the 484-capability registry. Required tests cannot be skipped, weakened, mocked at the final boundary, or changed merely to accept broken behavior. Every important feature needs a mutation or negative proof that the test fails when real behavior is broken.

## 13. Documentation update rules
S0 control laws are immutable during an executor run. S1 specs change only by requirement-scoped ADR plus invalidation. S2 ExecPlans update only at milestone boundaries. S3 volatile status exists only in `.agent/state/` and evidence directories.

## 14. Security rules
Follow `SECURITY.md`. Invention content is Confidential/device-only by default. Never expose provider tokens, copy consumer browser sessions, log invention bodies to telemetry, bypass access controls, or permit an LLM/MCP tool to file, pay, publish, contact external parties, or mutate Git without the specified authorization gate.

## 15. Definition of Done
The following 42 clauses are the run law. Every applicable clause is evidence-backed; an unavailable capability is BLOCKED/DEFERRED/EXTERNAL_REQUIRED according to the verification harness, never silently passed.
...
All 484 registry IDs remain present. Each receives exactly one final accounted status with evidence and applicability reasoning. Blockers affect only proven dependents. Candidate epochs are immutable. Exact-artifact tests bind executable/installer digests. Verification stages V-000 through V-021 are defined under `.agent/verification/stage-plans/`.

## 17. Final response requirements
Report verified outcomes, remaining risks, blocked/external gates, exact artifact digests, commands run with exit status, test/accounting counts, deployment state, and the next lawful manual action if auto-deploy is disabled. Never report unrun work as passed.
