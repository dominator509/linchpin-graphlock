...
DISPOSITIONS: dict[str, tuple[str, str, str]] = {
    ...
    "DOD-006": ("FAIL", ".agent/evidence/DOD-007-collection-guard-finding.md",
                "Rust suite: 49 passed, 0 failed, 0 ignored (verified: zero #[ignore], zero 'filtered out'). But the three JS packages contained ZERO test files and vitest was invoked with --passWithNoTests, so `pnpm -r test:unit` reported success on an empty suite. Flag corrected to --passWithNoTests=false this session; the runner now exits 1. DOD-006 cannot be PASS while required JS-side coverage is absent."),
    "DOD-007": ("PARTIAL", ".agent/evidence/DOD-007-collection-guard-finding.md",
                "VIOLATION FOUND AND FIXED. All three JS packages reported 'No test files found, exiting with code 0' because every test:unit script passed --passWithNoTests; `pnpm -r test:unit` therefore exited 0 with zero tests collected. Corrected to --passWithNoTests=false across root, apps/desktop, packages/ui and packages/contracts; the runner now exits 1 on an empty suite (verified). Not PASS: the JS suite genuinely contains zero test files, and scripts/test-collection-guard.sh is still not wired to an expected-count manifest for the Rust suite."),
    ...
    "DOD-041": ("FAIL", ".agent/verification/APPLICABILITY_MATRIX.csv",
                "The applicability matrix still holds a single ALL-484/EVALUATE_PER_REGISTRY/NOT_STARTED row. 449 of 484 IDs are conditional or evaluate and none has an evidence-attached applicability decision."),
