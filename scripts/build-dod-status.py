...
DISPOSITIONS: dict[str, tuple[str, str, str]] = {
    ...
    "DOD-003": ("PARTIAL", ".agent/evidence/sbom/linchpin.cdx.json",
                "MSI and NSIS both produced with pinned digests, and a REAL SBOM now exists: CycloneDX 1.5, 507 components (12 first-party, 495 third-party), generated by scripts/generate-sbom.py from cargo metadata --locked with no new dependency. THIRD_PARTY_NOTICES.md carries the grouped license inventory and artifact/candidate digests. Not PASS: artifacts remain unsigned (PF-015), no provenance attestation, and the SBOM covers the Rust graph only -- the JavaScript lockfile is inventoried separately, so a single merged cross-ecosystem SBOM is still open."),
