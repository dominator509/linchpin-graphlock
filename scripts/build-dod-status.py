...
DISPOSITIONS: dict[str, tuple[str, str, str]] = {
    ...
    "DOD-023": ("PARTIAL", ".agent/evidence/documentation/STATUS.md",
                "EXECUTED, not assumed. scripts/doc-exec.py extracts every command and referenced file path from the operator-facing documents (README, COMMANDS, DEPLOYMENT, OPERATIONS, ROLLBACK, RELEASE, ENVIRONMENT), runs the commands as written, and checks that referenced paths exist. RE-MEASURED this round, superseding the earlier '10 commands executed, 8 PASS': 22 executed, 6 FAILED, 1 skipped, 0 missing paths. All 6 failures are accounted for: scripts/security-check.sh fails on cargo-deny licenses (ADR-002, correctly reported rather than masked) and the SAME licences check cascades to scripts/dependency-audit.sh exit 4 and scripts/verify.sh exit 1; scripts/live-fire.sh exits 2 until EP-007; scripts/production-readiness-check.sh exits 1 because the verdict is NO_GO, which is correct behavior; pnpm audit exits 1 on 2 moderate advisories (GHSA-82fw-gwwq-j7x9 in vitest/@vitest/mocker, dev-only, patched in >= 4.1.11, below the enforced --audit-level high). Earlier rounds also fixed scripts/install.sh, which EXITED 1 while printing success -- the `set -eu` trailing-`&&`-test bug; that pattern was audited across ALL scripts and 16 instances in 7 files were rewritten as explicit ifs. COMMANDS.md now carries a measured 'Known command status' table so an operator reading the published commands is not misled. FOUR ROBUSTNESS DEFECTS FOUND AND FIXED IN THE VERIFIER ITSELF: (1) it documented its own command and therefore re-invoked itself, recursing until the per-command timeout fired -- now guarded by a self-reference exclusion; (2) a missing runner raised an uncaught FileNotFoundError and crashed the gate -- now classified FAIL/NOT_FOUND; (3) `pnpm` was reported NOT_FOUND although installed, because CreateProcess ignores PATHEXT and pnpm is a .cmd shim -- now resolved through shutil.which; (4) a timed-out desktop lane left linchpin-desktop.exe running and holding the artifact -- now bounded per command with best-effort straggler reaping. All four are mutation-proven by probes. Not PASS: no upgrade/rollback/deployment operator path is documented, because auto-deploy is disabled, so those instructions do not exist to execute; and 6 of the published commands do not exit 0 as written."),
