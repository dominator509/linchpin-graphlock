...
Derived by `scripts/completion-report.py` at 2026-09-17T19:40:52Z from the verification state. Every number below is read from a state file, not transcribed.

## Release verdict

- Machine-validated verdict: **NO_GO**
- Blocking clauses: DOD-014=PARTIAL
- Candidate commit: `1935a6831ff0cb74f11d6e723d649c097e7aa90c` (base `3ee9c3f41795`)
- Working tree at manifest time: DIRTY (25 changed files)
- Epoch: `f58a4f3e286cbecd` over 153 tracked inputs
...
- **484 registry capabilities accounted**, one status each ({'PASS': 57, 'NOT_APPLICABLE': 410, 'EXTERNAL_REQUIRED': 7, 'PARTIAL': 5, 'NOT_RUN_BLOCKED_MATERIAL': 4, 'DEFERRED_LONG_RUNNING': 1})
...
- Artifacts bound to this run: executable `00286e822ad93a84…` (12500992 bytes), MSI `64fc4bbc0c1bb3aa…` (5709824 bytes)
...
- **Update/rollback across versions is unexecuted** because only one version exists; the lanes install
  ...
  upgrade.
- **Registry capabilities**: 4 of 484 capabilities are `NOT_RUN_BLOCKED_MATERIAL`, each naming the specific material its subject requires (an authorization surface, an isolation boundary, a git hook, hardware key storage), and 410 are `NOT_APPLICABLE` on per-ID applicability evidence.
